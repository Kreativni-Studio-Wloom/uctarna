...
export const DispatchView: React.FC<DispatchViewProps> = ({ storeId }) => {
	...
	useEffect(() => {
		...
		const q = query(salesRef, where('served', '==', false));
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<h2 className="text-2xl font-bold text-gray-900 dark:text-white">Výdej</h2>
				<div className="text-sm text-gray-500 dark:text-gray-400">Nevydané: {orders.length}</div>
			</div>

			{orders.length === 0 ? (
				<div className="text-center py-12">
					<Timer className="h-16 w-16 text-gray-400 mx-auto mb-4" />
					<h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Žádné objednávky k výdeji</h3>
					<p className="text-gray-600 dark:text-gray-400">Nové objednávky se zde zobrazí automaticky</p>
				</div>
			) : (
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{orders.map((order) => (
						<div key={order.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
